...
TOOL_START_LABELS: dict[str, Callable[[dict[str, Any]], str]] = {
    ...
    "create_query": lambda a: f"Creating query “{_truncate(a.get('name', ''), 40)}”",
    "update_query": lambda a: f"Updating query #{a.get('query_id')}",
    "archive_query": lambda a: f"Archiving query #{a.get('query_id')}",
    "create_visualization": lambda a: f"Creating {a.get('type', 'viz')} “{_truncate(a.get('name', ''), 36)}”",
