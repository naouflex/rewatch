...
from redash.destinations.discord_webhook import DEFAULT_EMBED_TITLE, DiscordWebhook
...
def test_discord_webhook_sends_default_embed():
    ...
    with mock.patch("redash.destinations.discord_webhook.requests.post") as mock_post:
...
def test_discord_webhook_falls_back_when_custom_body_not_json():
    ...
    options = {"url": "https://discord/webhook", "username": "redash-bot", "avatar_url": "http://img"}
    alert = _alert(custom_body=custom)
    ...
    assert sent["username"] == "redash-bot"
